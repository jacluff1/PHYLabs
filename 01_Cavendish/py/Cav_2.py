...
def G(theta,k,R,M_m,m_m,d,static=True): # gravitational constant - N m^2 kg^-2
    ...
    err = np.sqrt( (k.err/k.val)**2 + (th_err/th_val)**2 + 2*(R.err/R.val)**2 + (M_m.val/M_m.err)**z + (m_m.val/m_m.err)**z + (d.val/d.err)**z )
