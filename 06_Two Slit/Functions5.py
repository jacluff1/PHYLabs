...
def PLOTS(figN,binsize,TITLE,arg):
    ...
    T = linspace(min(X),max(X),N_plot)
    T_BIN = linspace(min(X),max(X),binsize+1)
    dT = (max(X)-min(X))/(binsize+1)
    ...
    K = array(K).astype(int)
    K_N = normalize(K)
    Lambda = COUNT/LENGTH
    
    #axis.hist(K, T_BIN, facecolor = 'b', normed = True, alpha = .6)
    ...
    def gaussian(X,Y):
        ...
        def FIT(X,Y):
            ...
            qout,success = optimize.leastsq(errfunc, [85,100], args=(xdata, ydata),maxfev=5000)

            out = qout[:]
            out[0] = qout[0]
            out[1] = qout[1]
            return out[0],out[1]
        #var,mean = FIT(T_BIN,K_N)
        #print(var,mean)
        ...
        def FUNC(x,variance,mean):
            ...
            two = np.exp(two_1/two_2)
            return one*two
        return FUNC(T,variance,mean)
    YG = gaussian(T_BIN,K_N)
    
    fig = plt.figure(figN,figsize=figS)
    plt.xlim([min(X),max(X)])
    plt.xlabel('t (s)',fontsize=fontS)
    plt.ylabel('$ %s Counts$' % arg,fontsize=fontS)    
    
    plt.hist(K,T_BIN,facecolor='g',normed=True)
    plt.plot(T_BIN,poisson(K_N),'b',linewidth=5)
    plt.plot(T,YG,'r',linewidth=5)
